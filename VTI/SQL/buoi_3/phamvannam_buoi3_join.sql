create database phamvannam_buoi3_join;
use phamvannam_buoi3_join;
...
on acc.PositionID=po.PositionIDX
...
select dp.DepartmentID,dp.DepartmentName,po.PositionName,count(po.PositionID) from department as dp
inner join `account` as acc
...
select TQ.typeID,TQ.TypeName AS LoaiCauHoi,count(Q.QuestionID) as 'soluongcauhoi' from  TypeQuestion AS TQ 
