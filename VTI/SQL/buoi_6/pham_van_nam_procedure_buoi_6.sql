create database pham_van_nam_procedure_buoi_6;
use pham_van_nam_procedure_buoi_6;
...
-- viết 
DELIMITER $$
create procedure layalldepartment()
begin 
     select * from department;
end ;

call layalldepartment();

-- viết hàm thủ tục chèn vào phòng ban
delimiter $$ 
create procedure chenphongban(in DepartmentName_vl varchar(200))
begin
      insert into Department(DepartmentName)
      values(DepartmentName_vl);
end $$;
delimiter 


call chenphongban('Bảo Vệ Công Chúa');
delimiter 
...
-- Question 3: Tạo store để thống kê mỗi type question có bao nhiêu question được tạo trong tháng hiện tại em chưa biết làm

-- Question 4: Tạo store để trả ra id của type question có nhiều câu hỏi nhất 
-- select TypeID,count(QuestionID) as soluongcauhoi from question
-- group by TypeID
-- order by soluongcauhoi desc
-- limit 1;

select ty.TypeID,count(qe.QuestionID) as cauhoi from typequestion as ty
inner join question as qe
on ty.TypeID=qe.TypeID
group by ty.TypeID
having count(QuestionID)=
					(select count(QuestionID) as soluongcauhoi from question
					group by TypeID
					order by soluongcauhoi desc
					limit 1);
                    
-- DELIMITER $$
-- create procedure layidconhieucauhoi()
-- begin 
-- 	select ty.TypeID,count(qe.QuestionID) as cauhoi from typequestion as ty
-- 	inner join question as qe
-- 	on ty.TypeID=qe.TypeID
...
-- DELIMITER ;
-- call layidconhieucauhoi();

-- 1. Xóa cái cũ đi
DROP PROCEDURE IF EXISTS layidconhieucauhoi;
